...
// ======================================================
// === START: TOP_SCORERS.JS (Page-Specific Logic) ===
// ======================================================
...
const topscorersContainer = document.getElementById('topscorers-container');
...
if (searchButton) {
    searchButton.addEventListener('click', () => {
        ...
        if (!leagueId || !season) {
            topscorersContainer.innerHTML = "<p>Please enter both a League ID and a Season.</p>";
            return;
        }

        topscorersContainer.innerHTML = "<p>Loading top scorers...</p>";

        // Build the correct URL for top scorers
        const apiUrl = `https://v3.football.api-sports.io/players/topscorers?league=${leagueId}&season=${season}`;

        fetch(apiUrl, fetchOptions)
            .then(handleResponse)
            .then(data => {
                console.log('Top Scorers Results:', data);
                if (data.errors && (data.errors.required || data.errors.league || data.errors.requests)) {
                    ...
                    topscorersContainer.innerHTML = `<p>Error: ${errorMsg}</p>`;
                } else {
                    // Send data to the display function
                    displayTopScorers(data.response, topscorersContainer);
                }
            })
            .catch(error => {
                console.error('Fetch error:', error);
                topscorersContainer.innerHTML = `<p>An error occurred while fetching data. ${error.message}</p>`;
            });
    });
}

// --- FUNCTION to display top scorers cards ---
function displayTopScorers(players, containerElement) {
    if (!containerElement) return;

    containerElement.innerHTML = "";
    if (!players || players.length === 0) {
        containerElement.innerHTML = "<p>No top scorers found for this league and season.</p>";
        return;
    }
    ...
    players.forEach(item => {
        ...
        // Build the stats card (with focus on Goals)
...
// ======================================================
// === END: TOP_SCORERS.JS (Page-Specific Logic) ===
// ======================================================